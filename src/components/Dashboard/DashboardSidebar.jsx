    ...
    <div className='basis-1/5'>
      <ul className='flex flex-col gap-1'>
        {sections.map(section=>
            <Link key={section.value} to={`/dashboard/${section.value}`} className={`p-2 transition-all duration-150 ease-in hover:text-orange-300 ${active===section.value ? "bg-gray-300 font-bold" : ""}`}>
                {section.label}
            </Link>
        )}
