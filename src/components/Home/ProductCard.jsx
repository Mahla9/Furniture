...
function ProductCard({product,list,carousel}) {
  ...
  return (
    <div key={product.productId} className={`relative ${carousel ? "w-48 sm:min-w-60 md:min-w-52 shrink-0" : "" } bg-white flex  ${list === "col" ? "flex-col hover:-translate-y-3" : "flex-row"} rounded-lg shadow-lg shadow-gray-300 overflow-hidden transition-transform duration-150 ease-in group/card`}>
        ...
        {/* image card */}
        <div className={`relative w-full aspect-square lg:translate-y-3 transition-all duration-150 ease-in flex justify-center cursor-pointer ${list==="col" ? "lg:group-hover/card:translate-y-0" : ""}`} onClick={()=>navigate(`/product/${product.productId}`)}>
