  ...
  return (
    <div className='w-full h-[calc(100vh-350px)] sm:h-[calc(100vh-300px)] md:h-[calc(100vh-140px)] relative flex'>
      {slides.map((slide, index) => (
        <div 
          key={index}
          className={`pl-14 lg:pl-28 absolute inset-0 flex flex-col justify-around items-start transition-all ease-linear duration-500 overflow-hidden
          ${currentSlide===index ? "translate-x-0" : "-translate-x-full"}`}
        >
          {/* Background Image به صورت <img> */}
          <img 
            src={slide.image}
            alt={`${slide.text} by ${slide.artist}`}
            className="absolute inset-0 w-full h-full object-cover object-center -z-10"
            // فقط اولین اسلاید fetchpriority="high" داره
            fetchPriority={index === 0 ? "high" : "low"}
            loading={index === 0 ? "eager" : "lazy"}
          />

          {/* محتوای اسلاید */}
          <div className='flex gap-2 items-center text-gray-700 font-semibold relative z-10'>
              <img 
                src={slide.icon} 
                alt={slide.category}
                className='p-4 rounded-full bg-black bg-opacity-10 aspect-square object-center'
                loading={index === 0 ? "eager" : "lazy"}
              />
          ...
          <div className='font-bold relative z-10'>
              <div className='text-lg sm:text-xl md:text-2xl lg:text-3xl xl:text-4xl text-orange-500 md:text-gray-700'>
                ...
                <div className='flex'>
                  by <img 
                    className='mx-3 w-[35px] sm:w-[40px] md:w-auto' 
                    src={slide.avatar} 
                    alt={slide.artist}
                    loading={index === 0 ? "eager" : "lazy"}
                  /> {slide.artist}
              ...
              <div className='mt-4 flex gap-3 items-center group text-gray-700'>
                  <button type='button' className='bg-white text-xs md:text-xl px-2 md:px-3.5 py-1 md:py-2.5 rounded-3xl transition-all ease-in duration-150 group hover:bg-gray-200'>
                    Shop Now
                  </button>
      ...
      <div className='absolute bottom-2 left-1/2 -translate-x-1/2 z-20 flex gap-1.5 bg-gray-100 px-2 py-1 rounded-xl'>
          {slides.map((_, index) => (
            <span 
              key={index}
              className={`w-2.5 h-2.5 rounded-full ${currentSlide===index ? "bg-gray-600" : "bg-gray-300"} cursor-pointer`} 
