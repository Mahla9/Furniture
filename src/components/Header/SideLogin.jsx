      ...
      <div className={`fixed right-0 top-0 h-screen z-50 w-64 md:w-80 bg-white transition-all duration-200 ease-linear ${showSideLogin?"translate-x-0":"translate-x-full"}`}>
        ...
        <div className='px-4 py-6'>
          <form typeof='login' onSubmit={handleSubmit(onSubmit)} className='flex flex-col gap-3'>
            ...
            <button type='submit' className={`py-2 rounded-full text-white transition-all duration-150 ease-in ${loading?"disabled bg-orange-300":"bg-orange-400 hover:bg-orange-500"}`}> Log in {loading && <Spin size='small'/> }  </button>
          </form>
          ...
          <div className='flex items-center justify-between my-6'>
            ...
            <span onClick={()=>navigate('/forgot-password')} className='text-orange-400 text-sm cursor-pointer'>Forgot your password?</span>
          </div>
        </div>
